...
class MeasurementCounter extends StatelessWidget {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final count = snapshot.data!.docs
            .map((doc) => Measurement.fromJson(
                doc.data() as Map<String, dynamic>, doc.id))
            .where(filter)
            .length;

        return _buildCount(context, count);
      },
    );
  }

  Widget _buildCount(BuildContext context, int count) => Text(
        '$count',
  ...
  Widget _buildError() => const Text(
        '---',
        style: TextStyle(color: AppColors.blue1),
      );
}

class GraphStatistics extends StatelessWidget {
  const GraphStatistics({super.key});
      ...
      children: [
        ...
        _buildCounterCard(
          ...
          counter: MeasurementCounter(
            ...
            filter: (measurement) =>
                measurement.uploaderId == currentUser?.uid,
          ),
        ),
        _buildCounterCard(
          context,
        ...
        child: Row(
          children: [
            CircleAvatar(
              ...
              child: Icon(icon, color: AppColors.blue1),
            ),
